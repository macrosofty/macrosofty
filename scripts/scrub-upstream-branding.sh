...
# --- Aurora offline-docs entry + aurora.pdf --------------------------------
# Aurora ships /usr/share/applications/dev.getaurora.offline-docs.desktop
# (Exec=xdg-open /usr/share/doc/aurora/aurora.pdf) and the PDF itself at
# /usr/share/doc/aurora/aurora.pdf. The Name= field gets brand-scrubbed
# above to "Macrosofty Offline Docs" but the menu entry still opens
# Aurora's PDF — that surfaces upstream brand to the user. We don't ship
# a Macrosofty PDF (docs strategy is app-based via AppHelperThingy with
# a PDF-rewrite fallback if needed — see project_docs_strategy memory).
#
# Drop the offline-docs entry and the PDF entirely. The online
# Documentation entry (Exec=xdg-open https://docs.getaurora.dev) is
# kept; its URL gets rewritten to ours by the URL-substitution pass
# above, and its Icon= gets handled in the icon-override section below.
rm -f /usr/share/applications/dev.getaurora.offline-docs.desktop
rm -rf /usr/share/doc/aurora
echo "Removed Aurora offline-docs entry + aurora.pdf"

# --- Override Aurora-named icon files in-place -----------------------------
# Aurora ships icon files at canonical paths that .desktop entries
# reference by name (Icon=aurora-docs, Icon=aurora-update, etc.).
# Rewriting Icon= lines in every .desktop file means tracking every
# entry; instead, we keep the filenames and replace the SVG contents
# with our logo. Same Icon= line, different visual.
#
# Source file: /usr/share/icons/hicolor/scalable/apps/macrosofty.svg —
# shipped via system_files/shared/. If it's missing, we skip rather
# than break the build.
MACROSOFTY_LOGO=/usr/share/icons/hicolor/scalable/apps/macrosofty.svg
if [ -f "$MACROSOFTY_LOGO" ]; then
    # Apps icons (referenced by Aurora's .desktop entries)
    for target in aurora-docs aurora-update aurora-discussions; do
        dest="/usr/share/icons/hicolor/scalable/apps/${target}.svg"
        if [ -f "$dest" ]; then
            cp -f "$MACROSOFTY_LOGO" "$dest"
            echo "Icon override: ${target}.svg -> macrosofty.svg"
        fi
    done

    # Places icons (auroralogo-* used in panel/menu UIs and as
    # distributor-logo fallbacks).
    for target in auroralogo-white auroralogo-circle-symbolic \
                  auroralogo-pride auroralogo-gradient \
                  auroralogo-pride-trans auroralogo-symbolic; do
        dest="/usr/share/icons/hicolor/scalable/places/${target}.svg"
        if [ -f "$dest" ]; then
            cp -f "$MACROSOFTY_LOGO" "$dest"
            echo "Icon override: ${target}.svg -> macrosofty.svg"
        fi
    ...
    echo "Warning: $MACROSOFTY_LOGO missing; skipping aurora-named icon overrides"
fi

# Refresh the icon cache so the renamed-content icons take effect
# without a logout. gtk-update-icon-cache walks the theme tree and
# rebuilds icon-theme.cache; safe to skip if not installed.
