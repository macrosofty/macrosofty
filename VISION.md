...
### 🧺 Macrosofty Padkos
**Old hardware. Patchy WiFi. No problem.**

Padkos is the food you take for the road — what keeps you going on the long drive. This edition does the same for a laptop with intermittent internet, or an older machine that should still feel useful. Boots fast on 4 GB. Browser, email, documents, video — all on disk from day one, before the WiFi even shows up. Quiet, dignified, ready when you are.
...
- **South African flavour, sparingly.** The voice carries a South African pinch; one *lekker*, one *howzit*, one *boet* per page is flavour. Every sentence would be cosplay.
...
> **"I'm reviving an older laptop, or I'm somewhere with patchy internet."**
> → You want **Macrosofty Padkos**. Runs on 4 GB. Boots in seconds. Browser + office on disk from day one — works the moment the screen lights up, even before WiFi.
...
| Upstream donors | Aurora (Hearty), Aurora-DX (Chunky), Aurora + offline RPMs (Padkos), Bazzite (Braai) | Inherit, don't reinvent |
...
- **Runs at a loss until it can't.** Domains and any overflow are paid out of pocket.
...
- **Afrikaans translation is the first one we'll add** when there's bandwidth.
...
- Minimum spec we publish: 3 GB RAM (Padkos — tested on less), 8 GB RAM (Hearty/Chunky/Braai), x86-64 CPU for Hearty/Chunky/Padkos (anything from ~2010+), x86-64-v3 for Braai (Bazzite's gaming kernel needs it — roughly 2013+), 30 GB disk, UEFI boot. We test against a couple of reference machines.
