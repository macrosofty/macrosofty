...
- [`SESSION_NOTES_2026-05-04.md`](./SESSION_NOTES_2026-05-04.md) — AppHelperThingy: Tjopper mascot + 7 expressions
- [`SESSION_NOTES_2026-05-02.md`](./SESSION_NOTES_2026-05-02.md) — Satellite-project scaffolder (`scripts/new-project.sh` + templates) + AppHelperThingy CLAUDE.md.
- [`SESSION_NOTES_2026-04-28.md`](./SESSION_NOTES_2026-04-28.md) — netinstall CI flake fix (retry on Fedora registry pull); padkos netinstall ISO downloaded + checksum-verified; durable token-load process built (`~/.config/macrosofty/load-tokens.sh` exports GitHub + GHCR + Cloudflare); persistent podman GHCR auth restored (no longer tmpfs-wiped on reboot); website audit checklist updated to drop "netinstall" jargon in favour of "Online installer (~1.2 GB · needs internet)" / "Full installer (~5 GB · works offline)".
- [`SESSION_NOTES_2026-04-26.md`](./SESSION_NOTES_2026-04-26.md) — public-flip + ISO pipeline working 4/4; full Aurora-branding rebrand pipeline (kickoff icon, Plymouth, hostname, MOTD, ublue-os tree, Anaconda product, .desktop entries, profile.d shell greetings, /etc/*-release); centralised brand identity to `config/identity.env`; per-edition OCI substitution; netinstall ISO pipeline working first try (4/4); Padkos LibreOffice RPM; theme-pack system + macrosofty-theme apply script; ranked value-adds roadmap (38 items, 6 tiers); Share & Serve project proposal (full spec); website roadmap page + DownloadWizard (committed local, not pushed); KDE-everywhere validated by Padkos 1-CPU/3-GB QEMU test.
